...
object UPSDeliveryService : DeliveryService {
    ...
    override suspend fun getParcel(
        ...
        val resp = try {
            service.getStatus(
                locale, "X-CSRF-TOKEN=${tokens.first}", tokens.second, GetStatusRequest(
                    locale, listOf(trackingId.lowercase())
                )
            )
        ...
        val history = details.shipmentProgressActivities!!.map {
            ...
            val adjustedDate = gmtDate.atOffset(ZoneOffset.UTC)
        ...
        val status = when (details.progressBarType) {
    ...
    private val retrofit =
        Retrofit.Builder().baseUrl("https://webapis.ups.com/track/api/").client(api_client)
    ...
    private suspend fun getCsrfTokens(trackingId: String): Pair<String, String> {
        val request = Request.Builder().addHeader("accept", "*/*")
            .addHeader("accept-language", "en-US,en;q=0.9").addHeader(
                "sec-ch-ua",
                "\"Not(A:Brand\";v=\"99\", \"Google Chrome\";v=\"133\", \"Chromium\";v=\"133\""
            ).addHeader("sec-ch-ua-mobile", "?0").addHeader("sec-ch-ua-platform", "\"Linux\"")
            .addHeader("sec-fetch-dest", "empty").addHeader("sec-fetch-mode", "cors")
            .addHeader("sec-fetch-site", "same-site").addHeader(
                "user-agent",
                "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/133.0.0.0 Safari/537.36"
            ).url("https://www.ups.com/track?tracknum=${trackingId.lowercase()}").build()
