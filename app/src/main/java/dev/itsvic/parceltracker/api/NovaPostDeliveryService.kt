...
object NovaPostDeliveryService : DeliveryService {
    ...
    override suspend fun getParcel(
        ...
        val history = resp.tracking.reversed().map {
            ParcelHistoryItem(
                it.event_name,
        ...
        val status = when (resp.tracking.last().code) {
            "1" -> Status.Preadvice
            "4", "6", "5" -> Status.InTransit
            "7", "8", "9" -> Status.Delivered
    ...
    private val retrofit =
        Retrofit.Builder().baseUrl("https://api.novapost.com/site/v.1.0/").client(api_client)
