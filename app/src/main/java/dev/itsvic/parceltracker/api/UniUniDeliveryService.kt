...
object UniUniDeliveryService : DeliveryService {
    ...
    override suspend fun getParcel(
        ...
        val history = parcel.spath_list.map {
        ...
        val status = when (parcel.state) {
            190, 223 -> Status.Preadvice
            192, 198 -> Status.Customs
            199, 200, 218, 221, 229 -> Status.InWarehouse
            195, 204, 217, 225, 255 -> Status.InTransit
            202 -> Status.OutForDelivery
            203, 228 -> Status.Delivered
            206, 207, 209, 211, 212, 213, 215, 222, 224, 230, 231, 232 -> Status.DeliveryFailure
            214, 226 -> Status.AwaitingPickup
            216 -> Status.PickedUp
    ...
    private val retrofit =
        Retrofit.Builder().baseUrl("https://delivery-api.uniuni.ca/cargo/").client(api_client)
    ...
    internal data class GetParcelResponse(
        val data: GetParcelResponseData
    )
