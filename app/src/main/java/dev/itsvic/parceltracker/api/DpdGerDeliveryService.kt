...
object DpdGerDeliveryService : DeliveryService {
    ...
    override suspend fun getParcel(trackingId: String, postCode: String?): Parcel {
        ...
        val status = when(val current = resp.parcellifecycleResponse.parcelLifeCycleData.statusInfo.find {
            it.isCurrentStatus == true
        }?.status) {
            "ACCEPTED" -> Status.Preadvice
            "ON_THE_ROAD" -> Status.InTransit
            "HANDOVER_CONSIGNOR_TO_PARCELSHOP", "AT_DELIVERY_DEPOT" -> Status.InWarehouse
            "OUT_FOR_DELIVERY" -> Status.OutForDelivery
            "DELIVERED" -> Status.Delivered
            else -> logUnknownStatus("DPD (Europe)", current!!)
        }

        val h = resp.parcellifecycleResponse.parcelLifeCycleData.statusInfo.filter {
    ...
    private val retrofit =
        Retrofit.Builder().baseUrl("https://tracking.dpd.de/rest/plc/en_DE/").client(api_client)
    ...
    private interface API{
        @GET("{id}")
        suspend fun getShipment(@Path("id") trackingId: String) : ParcelData
    }
    ...
    @JsonClass(generateAdapter = true)
    internal data class ParcelLifeCycleResponse(
