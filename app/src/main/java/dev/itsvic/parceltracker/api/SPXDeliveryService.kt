...
object SPXThailandDeliveryService : SPXDeliveryService(
...
open class SPXDeliveryService(
    ...
    override suspend fun getParcel(trackingId: String, postalCode: String?): Parcel {
        ...
        val history = resp.data.tracking_list.map {
            ParcelHistoryItem(
                // shit patch for their shit bug
                it.message.replace("\\\\n", ""),
                LocalDateTime.ofInstant(
                    Instant.ofEpochMilli(it.timestamp * 1000L),
        ...
        val status = when (resp.data.current_status) {
