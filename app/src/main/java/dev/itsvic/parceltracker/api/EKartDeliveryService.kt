...
object EKartDeliveryService : DeliveryService {
    ...
    override suspend fun getParcel(
        ...
        val status = when {
            ...
            resp.shipmentTrackingDetails.last().statusDetails == "Out For Delivery" -> Status.OutForDelivery
        ...
        val history = resp.shipmentTrackingDetails.reversed().map {
            ParcelHistoryItem(
                it.statusDetails, localDateFromMilli(it.date), it.city
            )
        }

        val properties = mapOf(
            (if (status == Status.Delivered) R.string.property_delivery_time else R.string.property_eta) to localDateFromMilli(
                resp.expectedDeliveryDate
            ).format(
        ...
        return Parcel(
            trackingId, history, status, properties
        )
    }

    private val retrofit =
        Retrofit.Builder().baseUrl("https://ekartlogistics.com/ws/").client(api_client)
    ...
    private interface API {
        ...
        suspend fun getTrackingDetails(
            @Body body: GetTrackingDetailsRequest
        ): TrackingDetails
    }
